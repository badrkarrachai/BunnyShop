    ...
    <>
      <div className=" px-6 sm:px-20 lg:px-32 flex flex-col-reverse lg:flex-row group h-full items-center w-full mb-28 lg:mb-0 gap-14">
        <div className="lg:w-[50%] w-full flex flex-col  lg:items-start gap-5 ">
            <div className="text-blue-gray-500 font-poppins lg:inline-block hidden dark:text-[#8993A5]"><p>Home <span className="mx-2">/</span> Products</p></div>
            <div className="text-xl sm:text-3xl  font-semibold sm:font-bold text-center lg:text-left capitalize dark:text-[#F1F2F4]">{title}</div>
            <div className=' flex gap-4 w-full lg:justify-start justify-center items-center'>
              <div className="text-md sm:text-xl font-bold sm:font-semibold dark:text-[#F1F2F4]">{price}</div> <span className="dark:text-[#B3B9C4]">|</span> <div className='text-blue-gray-500 dark:text-[#B3B9C4]'>{category}</div></div>
            <div className=" flex text-center lg:text-left text-blue-gray-500 dark:text-[#DCDFE4]">{description}</div>
            <div className='w-full justify-center lg:justify-start flex gap-2'><div className="flex items-center">{icon}</div><div className='text-blue-gray-500 dark:text-[#B3B9C4]'>{note}</div></div>
            <Button size="md" placeholder={undefined} style={{textTransform:"initial"}} className="cursor-pointer w-full mt-8 lg:mt-14 dark:bg-[#B0926A] bg-[rgb(19,23,39)] h-12 text-base">Add to cart</Button>
            <div className="flex justify-center w-full gap-2"> <div className="flex items-center"><IoShieldOutline size="1.2rem" color="#6F7D8B" /></div> 
            <div className="text-md dark:text-[#8993A5] font-bold sm:font-semibold flex gap-4 text-blue-gray-500">{gurantees}</div></div>
        </div>
